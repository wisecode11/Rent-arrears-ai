 ...
 */
function filterNonRentalChargesFromLastZero(nonRentalCharges, lastZeroDate, ledgerEntries) {
    if (!lastZeroDate) {
    ...
    if (ledgerEntries && ledgerEntries.length > 0) {
        ...
        if (lastZeroIndex >= 0) {
            ...
            const nonRentalDescriptionsAfterZero = new Set();
...
function generateExcelFile(data) {
    ...
    const filteredNonRentalCharges = filterNonRentalChargesFromLastZero(data.nonRentalCharges, data.lastZeroOrNegativeBalanceDate, data.ledgerEntries);
    ...
    if (data.lastZeroOrNegativeBalanceDate) {
        nonRentalChargesData.unshift([`Note: Showing only non-rental charges from last zero/negative balance date (${data.lastZeroOrNegativeBalanceDate})`, '', '', '']);
 ...
 */
function generateExcelFilename(data) {
