...
exports.classifyDescription = classifyDescription;
exports.parseMoney = parseMoney;
exports.parseFlexibleDate = parseFlexibleDate;
exports.parseLedgerFromText = parseLedgerFromText;
exports.chargesFromLedgerEntries = chargesFromLedgerEntries;
...
function parseMoney(raw) {
    ...
    if (!cleaned.includes('.') && /^\d+$/.test(cleaned) && cleaned.length >= 5)
        return null;
...
const DATE_TOKEN_REGEX = /(\d{4}-\d{1,2}-\d{1,2}|\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4})/;
...
function stripTrailingMoneyTokens(line) {
    return line.replace(MONEY_TOKEN_REGEX, ' ').replace(/\s+/g, ' ').trim();
}
function parseLedgerFromText(text) {
    const lines = text
        .split('\n')
        .map((l) => l.trim())
        .filter((l) => l.length > 0);
    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        if (upper.includes('PAGE ') || upper.startsWith('TOTAL'))
            continue;
    ...
    if (entries.length < 3) {
        for (const line of lines) {
            if (line.match(DATE_TOKEN_REGEX))
                rejectedLines.push(line);
            if (rejectedLines.length >= 20)
                break;
        }
    }
...
function chargesFromLedgerEntries(ledgerEntries) {
    const rentalCharges = [];
