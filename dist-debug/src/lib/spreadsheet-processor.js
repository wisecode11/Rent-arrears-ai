...
exports.analyzeSpreadsheet = analyzeSpreadsheet;
...
function parseExcelDate(value) {
    ...
    return (0, ledger_parser_1.parseFlexibleDate)(asString);
}
...
function mapColumns(header) {
    ...
    const findCol = (keys) => {
        for (let i = 0; i < normalized.length; i++) {
            ...
            if (!cell)
                continue;
            if (keys.some((k) => cell === k || cell.includes(k)))
                return i;
        }
        return undefined;
    };
...
function guessLedgerSheet(workbook) {
    let best = null;
    for (const sheetName of workbook.SheetNames) {
        ...
        const score = (cols.date !== undefined ? 2 : 0) +
...
function findOpeningBalanceFromRows(rows, maxScan = 25) {
    for (let r = 0; r < Math.min(maxScan, rows.length); r++) {
        const row = rows[r] || [];
        for (let c = 0; c < row.length; c++) {
            ...
            if (cell.includes('opening balance') || cell.includes('beginning balance') || cell.includes('year starting balance')) {
                ...
                for (let k = 1; k <= 3; k++) {
                    ...
                    const parsed = typeof candidate === 'number' ? candidate : (0, ledger_parser_1.parseMoney)(String(candidate ?? ''));
...
function analyzeSpreadsheet(buffer) {
    ...
    for (let r = headerRowIndex + 1; r < rows.length; r++) {
        ...
        if (!date)
            continue;
        const descriptionVal = cols.description !== undefined ? row[cols.description] : '';
        ...
        const debitParsed = typeof rawDebit === 'number'
            ? rawDebit
            : (0, ledger_parser_1.parseMoney)(String(rawDebit ?? '')) ?? 0;
        const creditParsed = typeof rawCredit === 'number'
            ? rawCredit
            : (0, ledger_parser_1.parseMoney)(String(rawCredit ?? '')) ?? 0;
        const amountParsed = typeof rawAmount === 'number'
            ? rawAmount
            : (0, ledger_parser_1.parseMoney)(String(rawAmount ?? '')) ?? 0;
        let debit = 0;
        let credit = 0;
        ...
        if ((debitParsed ?? 0) !== 0 || (creditParsed ?? 0) !== 0) {
            ...
            if (cls.isPayment || amountParsed < 0)
                credit = Math.abs(amountParsed);
            else
                debit = Math.abs(amountParsed);
        }
        let balance = 0;
        const balanceParsed = typeof rawBalance === 'number'
            ? rawBalance
            : (0, ledger_parser_1.parseMoney)(String(rawBalance ?? ''));
