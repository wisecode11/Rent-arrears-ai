...
exports.parseResidentLedgerFormat = parseResidentLedgerFormat;
exports.parsePDFTextDirectly = parsePDFTextDirectly;
exports.analyzeWithAI = analyzeWithAI;
exports.validateHuggingFaceConfig = validateHuggingFaceConfig;
const ledger_parser_1 = require("@/lib/ledger-parser");
function extractIssueDateISO(extractedText) {
    const toISO = (mm, dd, yyyy) => `${yyyy}-${mm.padStart(2, '0')}-${dd.padStart(2, '0')}`;
    ...
    const candidates = [];
    const addMatch = (re, score, source) => {
 ...
 */
function parseResidentLedgerFormat(extractedText) {
    ...
    const isBldgUnitResidentLedger = lines.some((l) => /Bldg\/Unit/i.test(l) && /Transaction\s*Date/i.test(l) && /Charges/i.test(l) && /Credits/i.test(l) && /Balance/i.test(l)) ||
    ...
    const normalizeBrokenDecimals = (input) => {
        let s = input;
    ...
    const extractTailMoneyTokens = (block) => {
    ...
    const isBldgUnitRowStart = (s) => DATE_FISCAL_ROW.test(s) && /RESIDENT/i.test(s);
    for (let i = dataStartIndex; i < lines.length; i++) {
        ...
        if (isBldgUnitResidentLedger) {
            if (!isBldgUnitRowStart(raw))
                continue;
        }
        else {
            if (!DATE_PREFIX_REGEX.test(raw))
                continue;
        }
        let buffer = raw;
        ...
        while (i + 1 < lines.length) {
            ...
            if (isBldgUnitResidentLedger) {
                if (isBldgUnitRowStart(next))
                    break;
            }
            else {
                if (DATE_PREFIX_REGEX.test(next))
                    break;
            }
            if (next.match(/^\d+\s*\/\s*\d+\s*$/))
                break; // page "1 / 8"
            if (next.toUpperCase().includes('RESIDENT LEDGER'))
                break;
            if (next.toUpperCase().startsWith('TOTAL'))
                break;
            buffer = `${buffer}\n${next}`;
            i++;
            wrapLines++;
    ...
    for (const line of coalescedLines) {
        ...
        const tailTokens = allTokens.length >= 3
            ? allTokens.slice(-3)
            : allTokens.length >= 2
                ? allTokens.slice(-2)
                : allTokens;
        ...
        if (tailTokens.length >= 3) {
            ...
            const looksLikePayment = chgCode === 'chk' ||
                amt < 0 ||
        ...
        const paymentLike = rawCode.toLowerCase().startsWith('pmt') ||
        ...
        let description = blockLines.length === 1
        ...
        const classified = (0, ledger_parser_1.classifyDescription)(description);
        ...
        const isNonRental = !isRental && !isPayment && !isCredit && !classified.isBalanceForward && (classified.isNonRentalCharge ||
            chgCode === 'latefee' ||
            chgCode === 'secdep' ||
            chgCode === 'nsf' ||
            chgCode === 'keyinc' ||
        ...
        if (isNonRental && debit > 0 && !isPayment && !isReversal) {
            ...
            if (chgCode === 'latefee')
                category = 'late_fee';
            else if (chgCode === 'secdep')
                category = 'security_deposit';
            else if (chgCode === 'nsf')
                category = 'bad_check';
            else if (chgCode === 'keyinc')
                category = 'lockout';
            else if (chgCode === 'uao')
                category = 'use_of_occupancy';
            else if (chgCode === 'utilele')
                category = 'utilities';
            else if (classified.category && classified.category !== 'rent')
    ...
    if (isBldgUnitResidentLedger && ledgerEntries.length === 0) {
        for (const raw of lines) {
            if (!DATE_FISCAL_ROW.test(raw) || !/RESIDENT/i.test(raw))
                continue;
            ...
            const tailTokens = tokens.length >= 3 ? tokens.slice(-3) : tokens.length >= 2 ? tokens.slice(-2) : tokens;
            if (tailTokens.length < 2)
                continue;
            let charge = 0;
            let payment = 0;
 ...
 */
function parseTenantLedgerFormat(extractedText) {
    const lines = extractedText.split('\n').filter(line => line.trim().length > 0);
    // Extract tenant name
    let tenantName = 'Unknown Tenant';
    ...
    const normalizeLedgerLine = (input) => {
        let s = input;
    ...
    for (let i = 0; i < Math.min(lines.length, 80); i++) {
        ...
        if (!upper.includes('STARTING BALANCE'))
            continue;
    ...
    for (let i = dataStartIndex; i < lines.length; i++) {
        ...
        if (line.match(/^Page\b/i))
            continue;
        if (line.match(/^Created on\b/i))
            continue;
        if (line.match(/^\d+\s*\/\s*\d+\s*$/))
            continue;
        if (line.toUpperCase().startsWith('TENANT LEDGER'))
            continue;
        if (line.toUpperCase().startsWith('DATE PAYER DESCRIPTION'))
            continue;
        ...
        description = description.replace(/^([A-Z][a-z]+\s+[A-Z][a-z]+)\s+(?=(?:ACH\s+Payment|Credit\s+Card\s+Payment|Payment|EFT|Wire|Check|Chk)\b)/, '').trim();
        ...
        const isNonRental = !isRental && !isPayment && debit > 0 && (classified.isNonRentalCharge ||
            isLateFee ||
            isSecurityDeposit ||
...
function parsePDFTextDirectly(extractedText) {
    const lines = extractedText.split('\n').filter(line => line.trim().length > 0);
    ...
    if (isResidentLedgerFormat) {
        ...
        const hasResidentData = (resident.ledgerEntries?.length ?? 0) > 0 ||
    ...
    const coalesceStatementLedgerLines = (text) => {
    ...
    const { ledgerEntries: ledgerEntriesRaw } = (0, ledger_parser_1.parseLedgerFromText)(extractedText);
    const { ledgerEntries: ledgerEntriesCoalesced } = (0, ledger_parser_1.parseLedgerFromText)(normalizedForLedger);
    const ledgerEntries = ledgerEntriesCoalesced.length > ledgerEntriesRaw.length ? ledgerEntriesCoalesced : ledgerEntriesRaw;
    const { rentalCharges, nonRentalCharges } = (0, ledger_parser_1.chargesFromLedgerEntries)(ledgerEntries);
    ...
    const period = ledgerEntries.length > 0
        ? `${ledgerEntries[0].date} to ${ledgerEntries[ledgerEntries.length - 1].date}`
        : 'Extracted Period';
    ...
    try {
        ...
        if (hasFinalBalance || hasCharges || hasLedgerEntries) {
            ...
            if (!hasFinalBalance && hasLedgerEntries && directResult.ledgerEntries) {
                const sorted = [...directResult.ledgerEntries].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                if (sorted.length > 0) {
 ...
 */
function createFallbackResponse(extractedText) {
    const lines = extractedText.split('\n').filter(line => line.trim().length > 0);
    ...
    const rentalCharges = [];
    const rentalRegex = new RegExp(`(\\d{2}\\/\\d{2}\\/\\d{4})\\s+\\d+\\s+BASE\\s+RENT\\s*:\\s*(${AMOUNT_WITH_CENTS})`, 'i');
    for (const line of lines) {
    ...
    const acRegex = new RegExp(`(\\d{2}\\/\\d{2}\\/\\d{4})\\s+\\d+\\s+AIR\\s+CONDITIONER\\s*:\\s*(${AMOUNT_WITH_CENTS})`, 'i');
    for (const line of lines) {
    ...
    const lateRegex = new RegExp(`(\\d{2}\\/\\d{2}\\/\\d{4})\\s+\\d+\\s+LATE\\s+(?:CHARGE|FEE)\\s*:.*?(${AMOUNT_WITH_CENTS})`, 'i');
    for (const line of lines) {
    ...
    const legalRegex = new RegExp(`(\\d{2}\\/\\d{2}\\/\\d{4})\\s+\\d+\\s+LEGAL\\s+FEES\\s*:.*?(${AMOUNT_WITH_CENTS})`, 'i');
    for (const line of lines) {
    ...
    const badCheckRegex = new RegExp(`(\\d{2}\\/\\d{2}\\/\\d{4})\\s+\\d+\\s+BAD\\s+CHECK\\s+CHARGE\\s*:.*?(${AMOUNT_WITH_CENTS})`, 'i');
    for (const line of lines) {
    ...
    const securityRegex = new RegExp(`(\\d{2}\\/\\d{2}\\/\\d{4})\\s+\\d+\\s+SECURITY\\s+DEPOSIT\\s*:\\s*(${AMOUNT_WITH_CENTS})`, 'i');
    for (const line of lines) {
