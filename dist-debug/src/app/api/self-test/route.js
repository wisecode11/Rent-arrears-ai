...
09/01/2025  latefee  Late Fee (08/2025)  50.00  0.00  1550.00  3
    ...
    const results = fixtures.map((f) => {
        const parsed = (0, ledger_parser_1.parseLedgerFromText)(f.text);
        const { rentalCharges, nonRentalCharges } = (0, ledger_parser_1.chargesFromLedgerEntries)(parsed.ledgerEntries);
        ...
        const processed = (0, business_logic_1.calculateFinalAmount)(aiData, new Date(f.asOfDate));
        const nonRentHasBalanceForward = nonRentalCharges.some((c) => c.description.toUpperCase().includes('YEAR STARTING BALANCE'));
        return {
            name: f.name,
            asOfDate: f.asOfDate,
            ...
            debugLedgerTail: f.name.startsWith('Issue Date')
                ? parsed.ledgerEntries.slice(-5)
                : undefined,
        };
    });
    const tenantLedgerFixtures = [
        {
...
Total 0.00
    ...
    const tenantLedgerResults = tenantLedgerFixtures.map((f) => {
        const aiData = (0, huggingface_client_1.parsePDFTextDirectly)(f.text);
        const processed = (0, business_logic_1.calculateFinalAmount)(aiData, new Date(f.asOfDate));
        return {
            name: f.name,
            asOfDate: f.asOfDate,
...
1769-14T11/30/2024112024RESIDENT431PMTOPACHWelcomeHomeACHPayment0.001,123.11-1,098.11
    ...
    const residentLedgerResults = residentLedgerFixtures.map((f) => {
        const aiData = (0, huggingface_client_1.parsePDFTextDirectly)(f.text);
        const residentDirect = (0, huggingface_client_1.parseResidentLedgerFormat)(f.text);
        const processed = (0, business_logic_1.calculateFinalAmount)(aiData, new Date(f.asOfDate));
        ...
        const sampleLine = allLines.find((l) => /RESIDENT/i.test(l) && l.includes('/') && /\d{4}/.test(l)) ??
    ...
    return server_1.NextResponse.json({ ok: true, results, tenantLedgerResults, residentLedgerResults });
}
